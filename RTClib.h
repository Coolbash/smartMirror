...
{
public:
    CDateTime (uint32_t t =0);
    CDateTime (uint16_t year, uint8_t month, uint8_t day,uint8_t hour =0, uint8_t min =0, uint8_t sec =0);
    ...
    CDateTime operator-(const TimeSpan& span);
    TimeSpan operator-(const CDateTime& right);
...
public:
    boolean begin(void);
    static void adjust(const CDateTime& dt);
...
public:
    boolean begin(void);
    void adjust(const CDateTime& dt);
...
class RTC_Millis {
public:
    static void begin(const CDateTime& dt) { adjust(dt); }
    static void adjust(const CDateTime& dt);
    static CDateTime now();
